...
固定调用 `lifecycle_5y_full_wm` v2.0.0：最近5年／全部可用历史分别判断A/B/D/E/H，一致才入围，冲突和不足历史单列。每个窗口同时使用日线指标、确认后的周K结构和月K趋势区间确认。每次扫描和任务manifest记录同一策略契约，任务拒绝发布契约不一致的结果。
...
执行顺序：重新获取CN/US股票快照 → 当前过滤范围内下载全历史日线 → 最近5年及全部可用历史分别判断A/B/D/E/H → 汇总所有一致候选 → 获取财务报表 → 每市场每行业综合前5。

指标为日K指标、周K确认转折和月K慢周期确认。A/H必须处于月线低位区，D必须处于月线高位且月均线向上，B/E必须分别满足相应的月线多头/空头反弹结构。现有过滤：价格≥1、成交额≥2000万；A股去除ST/退市名称，美股市值≥10亿美元。不是所有上市股票无条件全覆盖。行业名称映射沿用已有文件，可能过时，新增代码无法匹配时保留未分类。
