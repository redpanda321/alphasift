...
固定调用 `lifecycle_5y_full` v1.0.0：最近5年／全部可用历史分别判断A/B/D/E/H，一致才入围，冲突和不足历史单列。不要求周K/月K共振。每次扫描和任务manifest记录同一策略契约，任务拒绝发布契约不一致的结果。此前讨论的月K改版未采用。
...
指标为日K加周K转折，不含月K确认。现有过滤：价格≥1、成交额≥2000万；A股去除ST/退市名称，美股市值≥10亿美元。不是所有上市股票无条件全覆盖。行业名称映射沿用已有文件，可能过时，新增代码无法匹配时保留未分类。
