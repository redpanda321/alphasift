...
def classify_window(history: pd.DataFrame) -> dict:
    ...
    weekly = df.assign(date=dates).set_index("date").resample("W-FRI").agg(
    ...
    scores = {"A": f["a_score"], "H": f["h_score"], "B": 0., "D": 0., "E": 0.}
    eligible = {"A": f["a_eligible"], "H": f["h_eligible"], "B": False, "D": False, "E": False}
    evidence = {"D_date": f["cycle_high_date"], "D_high": f["cycle_high"],
                "D_age_sessions": d_age, "drawdown_pct": dd}
    ...
    if eligible["D"]:
        scores["D"] = round(min(100, 60 + 20 * (1-dd/12) +
                                 10 * (f["rsi14"] >= 65) + 10 * (f["macd_status"] == "weakening")), 2)
    ...
    if after_lows:
        ...
        eligible["E"] = bool(f["prior_runup_pct"] >= 100 and first_decline >= 20
            and 5 <= dd <= 50 and 5 <= rebound <= 50 and d_age <= 252
            and not after_highs and len(after_lows) == 1
            and float(c.pct_change(20).iloc[-1]) > 0
            and price < f["ma200"])
        if eligible["E"]:
            scores["E"] = round(65 + 15*min(rebound/20, 1) +
                                10*(f["ma200_slope_60d_pct"] < 0) +
                                10*(f["macd_status"] == "improving"), 2)
    ...
    for low_date, low in reversed(lows):
        ...
        if (rise >= 30 and 5 <= pullback <= 25 and subsequent_low > low
                and price > f["ma200"] and f["ma200_slope_60d_pct"] > 0
                and f["ma50"] > f["ma200"]):
            eligible["B"] = True
            scores["B"] = round(65 + 15*min(rise/60, 1) + 10*f["stabilized"] +
                                10*(f["macd_status"] == "improving"), 2)
            evidence["B_anchor_A"] = {"date": str(low_date.date()), "price": low,
                                      "rally_high_date": str(peak_date.date()), "pullback_pct": pullback}
            break
    matches = [s for s in scores if eligible[s]]
    return {"stage": matches[0] if len(matches) == 1 else "UNCLASSIFIED" if not matches else "AMBIGUOUS",
            "matches": matches, "scores": scores, "evidence": evidence,
            "history_start": f["history_start"], "history_sessions": f["history_sessions"],
            "as_of": f["data_as_of"], "price": f["lifecycle_price"],
...
def crosscheck(history: pd.DataFrame, *, as_of: str) -> dict:
    ...
    status = ("INSUFFICIENT_DISTINCT_HISTORY" if not comparable else
              "AGREEMENT" if same and five["stage"] in STAGES else
              "NO_MATCH" if same and five["stage"] == "UNCLASSIFIED" else "CONFLICT")
    stage = five["stage"] if status == "AGREEMENT" else None
    return {"strategy": strategy_contract(), "as_of": as_of, "five_year_boundary": str(start.date()),
            "five_year_span_available": bool(five_span), "full_has_older_data": extra_history,
            "status": status, "consensus_stage": stage,
            "consensus_score": min(five["scores"][stage], full["scores"][stage]) if stage else None,
            "five_year": five, "full_history": full,
...
def main(argv=None):
    ...
    parser.add_argument("--symbols", help="comma-separated Yahoo symbols; omitted means filtered market")
    ...
    if args.symbols:
        ...
        snapshot = fetch_snapshot_with_fallback(config.snapshot_source_priority, market=args.market,
        ...
        symbols = codes if args.market == "us" else [c + (
            ".BJ" if c.startswith(("4", "8", "920")) else ".SS" if c.startswith("6") else ".SZ") for c in codes]
    ...
    def one(symbol):
        try:
            ...
            return {"symbol": symbol, "retrieved_at": pd.Timestamp.now(tz="UTC").isoformat(),
    ...
    payload = {"strategy": strategy_contract(), "scope": scope, "snapshot_count": snapshot_count, "attempted": len(symbols),
               "as_of": as_of, "rows": rows}
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, ensure_ascii=False, indent=2, allow_nan=False), encoding="utf-8")
    print(json.dumps({"output": str(output), "as_of": as_of,
